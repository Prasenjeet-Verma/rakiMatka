document.addEventListener("DOMContentLoaded", () => {
  ...
  function resetSingleDigitGame() {
    // clear memory
    modeAmounts.OPEN.fill("");
    modeAmounts.CLOSE.fill("");

    // reset mode
    currentMode = "OPEN";
    betTypeSelect.value = "open";
    betTypeSelect.querySelector('option[value="open"]').disabled = false;

    // clear inputs
    grid.querySelectorAll(".amount-input").forEach((input) => {
      input.value = "";
      input.dataset.mode = "OPEN";
    });
  ...
  betTypeSelect.addEventListener("change", () => {
    // save current values
    grid.querySelectorAll(".amount-input").forEach((input) => {
      const num = Number(input.dataset.number);
      modeAmounts[currentMode][num] = input.value;
    });

    // switch mode
    currentMode = betTypeSelect.value.toUpperCase();

    // load values of new mode
    let total = 0;
    grid.querySelectorAll(".amount-input").forEach((input) => {
      const num = Number(input.dataset.number);
      input.value = modeAmounts[currentMode][num] || "";
  ...
  grid.addEventListener("input", (e) => {
    ...
    // save value to current mode
    modeAmounts[currentMode][num] = e.target.value;
  ...
  submitBtn.addEventListener("click", async () => {
    ...
    ["OPEN", "CLOSE"].forEach((mode) => {
      modeAmounts[mode].forEach((amount, number) => {
