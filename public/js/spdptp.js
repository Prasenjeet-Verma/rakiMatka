document.addEventListener("DOMContentLoaded", () => {
  ...
  async function syncServerTime() {
    ...
    } catch { console.error("Server time fetch failed"); }
  }
  ...
  /* ================= FORMAT BETS FOR BACKEND ================= */
  function formatBetsForBackend(bets) {
    const grouped = {}; // session-type-mainNo

    bets.forEach(b => {
      const key = `${b.session}-${b.choose}-${b.mainNo}`;
      ...
      if (!grouped[key]) {
        ...
        grouped[key].totalPoints += b.points;
      }
    });

    return Object.values(grouped).map(b => ({
      ...b,
      perUnderNosPoints: b.totalPoints / b.underNos.length
    }));
  }

  /* ================= UI REFRESH (OLD UI PRESERVED) ================= */
  function refreshUI() {
    tableBody.innerHTML = "";
    let totalAmount = 0;
  ...
  addBtn.addEventListener("click", () => {
    ...
    choose.forEach(type => {
      ...
      numbers.forEach(num => {
        ...
        else betsRegistry.push({
          digit: num,
          choose: type,
          session,
          points,
          mainNo: openDigit   // 👈 FIX
        });
      });
    });
  ...
  submitBtn.addEventListener("click", async () => {
    if (!betsRegistry.length) return showMessage("No bets ❌", "error");

    const backendBets = formatBetsForBackend(betsRegistry);
