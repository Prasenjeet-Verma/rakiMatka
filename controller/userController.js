...
exports.getPanaChart = async (req, res, next) => {
 ...
 const logo = await AdminLogo.findOne();
    ...
    results.forEach((result) => {
      ...
      const weekday = new Date(result.resultDate).toLocaleDateString("en-US", {
        weekday: "short",
      });
    ...
    const finalWeeks = Object.values(weeks).sort(
...
exports.placeSingleDigitBet = async (req, res) => {
  try {
    ...
    for (const b of bets) {
      ...
      // ✅ NEW MIN / MAX CHECK (Separate Message)
...
exports.placeSingleBulkDigitBet = async (req, res) => {
  try {
    ...
    for (const b of bets) {
      ...
      // ✅ NEW MIN / MAX CHECK (without touching old logic)
      if (b.amount < minBet || b.amount > maxBet)
        return res.json({
          success: false,
          message: `Bet amount must be between ₹${minBet} and ₹${maxBet} ❌`,
...
exports.placeJodiDigitBet = async (req, res) => {
  ...
  try {
    ...
    for (const bet of bets) {
      ...
      // ✅ NEW MIN / MAX CHECK (old logic untouched)
...
exports.placeDoublePannaBulkBet = async (req, res) => {
 ...
 if (amountPerUnderNo < minBet || amountPerUnderNo > maxBet) {
  return res.json({
    ...
    return res.json({
      success: true,
      message: `Bet placed successfully ₹${totalAmount} ✅`,
    });
  } catch (err) {
...
const { minBet, maxBet } = mainSettings;
  ...
  return res.json({
    ...
    message: `Bet amount must be between ₹${minBet} and ₹${maxBet} ❌`,
      ...
      const validDigits = pattern === "ODD" ? [1, 3, 5, 7, 9] : [0, 2, 4, 6, 8];
...
const { minBet, maxBet } = mainSettings;
    ...
    for (const bet of bets) {
      ...
      if (typeof openPanna !== "string" || !/^\d{3}$/.test(openPanna)) {
        return res.json({
          success: false,
      ...
      if (typeof closeDigit !== "number" || closeDigit < 0 || closeDigit > 9) {
        return res.json({
          success: false,
...
if (b.amount < minBet || b.amount > maxBet) {
  return res.json({
    ...
    return res.json({
      success: true,
      message: `STARLINE SINGLE PANNA Bet placed successfully ₹${totalAmount}`,
    });
  } catch (err) {
    console.error("❌STARLINE SINGLE PANNA ERROR:", err);
    return res.json({
      success: false,
      message: "Server error ❌",
    });
    ...
    for (const Model of betModels) {
      ...
      records.forEach((bet) => {
        ...
        bet.bets.forEach((b) => {
          ...
          if (b.amount !== undefined) amounts.push(Number(b.amount));

          if (b.totalAmount !== undefined) amounts.push(Number(b.totalAmount));

          if (b.totalPoints !== undefined) amounts.push(Number(b.totalPoints));
